...
const label = "app.exeligmos.sync-server";

export async function installServerService() {
  if (process.platform !== "darwin") {
    throw new Error("Automatic sync-server startup is currently supported on macOS.");
  }
  ...
  await writeFile(
    ...
    plist({
      root,
      stdout: path.join(logRoot, "sync-server.log"),
      stderr: path.join(logRoot, "sync-server.error.log"),
    }),
